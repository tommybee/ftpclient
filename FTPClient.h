...
namespace nsFTP
{
   ...
   public:
      ...
      CFTPClient(std::shared_ptr<IBlockingSocket> apSocket=nsSocket::CreateDefaultBlockingSocketInstance(),
      ...
      void SetFileListParser(std::shared_ptr<IFileListParser> apFileListParser);
...
// data members
   private:
      ...
      mutable std::shared_ptr<CRepresentation> m_apCurrentRepresentation;  ///< representation currently set

      std::shared_ptr<IBlockingSocket>         m_apSckControlConnection;   ///< socket for connection to FTP server
      std::shared_ptr<IFileListParser>         m_apFileListParser;         ///< object which is used for parsing the result of the LIST command
